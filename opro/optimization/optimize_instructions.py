...
_LLAMA_CKPT_DIR = flags.DEFINE_string(
    "llama_ckpt_dir", "", "The directory of llama checkpoint")

_LLAMA_TOKENIZER_PATH = flags.DEFINE_string(
    "llama_tokenizer_path", "", "The path of Llama tokenizer")

_SCORER = flags.DEFINE_string(
    "scorer", "llama-2-7b-chat-hf", "The name of the scorer LLM."
)
...
_USE_SAME_MODEL = flags.DEFINE_bool(
    "use_same_model", False, "Whether use the same model for scorer and optimizer"
)
...
def main(_):
    ...
    use_same_model = _USE_SAME_MODEL.value
    ...
    if scorer_llm_name in {"gpt-3.5-turbo", "gpt-4"}:
    ...
    elif scorer_llm_name in {"llama-2-7b-chat", "llama-2-7b-chat-hf"}:
        ...
        scorer_llama_model = prompt_utils.LlamaModel(model_name=scorer_llm_name,
                                                     ckpt_dir=llama_ckpt_dir, tokenizer_path=llama_tokenizer_path)
    else:
    ...
    if optimizer_llm_name in {"gpt-3.5-turbo", "gpt-4"}:
        assert openai_api_key, "The OpenAI API key must be provided."
        openai.api_key = openai_api_key
    elif optimizer_llm_name in {"llama-2-7b-chat", "llama-2-7b-chat-hf"}:
        assert llama_ckpt_dir, "The directory of Llama checkpoint must be provided."
        assert llama_tokenizer_path, "The path of Llama tokenizer must be provided."
        if use_same_model:
            print("Use same model for scorer and optimizer")
            optimizer_llama_model = scorer_llama_model
        else:
            optimizer_llama_model = prompt_utils.LlamaModel(model_name=optimizer_llm_name,
                                                            ckpt_dir=llama_ckpt_dir, tokenizer_path=llama_tokenizer_path)
    else:
    ...
    if dataset_name == "mmlu":
        root_data_folder_path = os.path.join(
            ROOT_DATA_FOLDER_PATH, "MMLU-data")
    elif dataset_name == "bbh":
        root_data_folder_path = os.path.join(
    ...
    elif scorer_llm_name.lower() in {"llama-2-7b-chat", "llama-2-7b-chat-hf"}:
        scorer_llama_temperature = 0.1
        scorer_llama_max_length = 1024
        scorer_llama_dict = dict()
        scorer_llama_dict["num_beams"] = 1
        scorer_llama_dict['temperature'] = scorer_llama_temperature
        scorer_llama_dict["max_length"] = scorer_llama_max_length
        scorer_llama_model.create_model(kwargs=scorer_llama_dict.copy())
    ...
    if optimizer_llm_name.lower() == "text-bison":
        ...
        optimizer_llama_dict["num_beams"] = 1
        optimizer_llama_dict['temperature'] = optimizer_llama_temperature
        optimizer_llama_dict["max_length"] = optimizer_llama_max_length
        if use_same_model:
            pass
        else:
            optimizer_llama_model.create_model(kwargs=optimizer_llama_dict.copy())
        optimizer_llama_dict["num_decodes"] = 1
        ...
        call_optimizer_server_func = functools.partial(
            optimizer_llama_model.call_llama
        )
    else:
    ...
    if dataset_name == "mmlu":
        ...
        tasks_all = [(folder_name, task_name)
