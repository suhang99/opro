...
def main(_):
    ...
    if scorer_llm_name in {"gpt-3.5-turbo", "gpt-4"}:
        ...
        scorer_llama_model = prompt_utils.LlamaModel(
    ...
    if dataset_name == "mmlu":
        ...
        root_data_folder_path = os.path.join(
            ROOT_DATA_FOLDER_PATH, "AQuA-data")
    else:
    ...
    for t in tasks_all:
        if dataset_name == "mmlu":
            ...
            prediction_treat_as_bool = False
            num_examples = len(raw_data)
            original_index = np.sort(
                np.array(list(raw_data.keys())).astype(int))
        elif dataset_name == "aqua":
            task_name = t
        ...
        for i_ins, instruction in enumerate(instructions_to_evaluate):
            ...
            if evaluate_test_fold:
                ...
                detailed_test_results_df.to_csv(
                    test_file_path, index=True, header=True)
            ...
            if evaluate_training_fold and evaluate_test_fold:
                ...
                detailed_all_results_df = pd.concat(
                    [detailed_train_results_df,
                        detailed_test_results_df]  # pylint: disable=undefined-variable
                )
                ...
                print(
                    f"saving training + test results to\n{train_and_test_file_path}")
